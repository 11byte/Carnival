...
querys = f'select event from vol_login'
...
eve1_title = eve_list[0]
eve2_title = eve_list[1]
eve3_title = eve_list[2]
eve4_title = eve_list[3]
eve5_title = eve_list[4]

my_tab2.pack(pady=10,expand=True,fill='both')
my_tab2.configure(fg_color='#9a82e1')
case_1 = my_tab2.add(eve1_title)
case_2 = my_tab2.add(eve2_title)
case_3 = my_tab2.add(eve3_title)
case_4 = my_tab2.add(eve4_title)
case_5 = my_tab2.add(eve5_title)
...
count_lst = []
for l in eve_list:
    mycursor.execute(f"SELECT max_p FROM eve_rules WHERE eve_name = '{l}'")
    lst = mycursor.fetchall()
    for m in lst:
        count_lst.append(m[0])
mapping_dict = dict(zip(eve_list, count_lst))
print(mapping_dict)




eve1_count = mapping_dict.get(eve_list[0])
title1 = eve_list[0].replace(' ', '_')
eve2_count = mapping_dict.get(eve_list[1])
title2 = eve_list[1].replace(' ', '_')
eve3_count = mapping_dict.get(eve_list[2])
title3 = eve_list[2].replace(' ', '_')
eve4_count = mapping_dict.get(eve_list[3])
title4 = eve_list[3].replace(' ', '_')
eve5_count = mapping_dict.get(eve_list[4])
title5 = eve_list[4].replace(' ', '_')

# for i in range(0,5):
#     eve_list[i] = eve_list[i].replace(' ', '_')
#     print(eve_list[i])
def eve1():
    if len(e12.get()) == 10:
...
ctk.CTkLabel(frame2,text='GDRIVE LINK FOR IMAGES:',text_color='gold',font=('Georgia',20)).place(x=2,y=450)
mydb = mysql.connector.connect(
    host='localhost',
    user='root',
    password='mysql022004',
...
mycursor = mydb.cursor()
mycursor.execute("select * from gdrive")
gd = mycursor.fetchall()
gd = gd[0]
print(gd[0])
ctk.CTkLabel(frame2,text=gd[0],text_color='white',font=('Georgia',20)).place(x=300,y=450)
